...
@inject(stores => ({
	currentUserBets: stores.rootStore.userStore.currentUserBets,
}))
@observer
class BetsHistory extends React.Component {
	static propTypes = {
		currentUserBets: PropTypes.array.isRequired,
	};
	render() {
		const { currentUserBets } = this.props;
		...
		return (
			<Container>
					...
					<Row>
						<Col>
							<p>
								...
								<Link to={pathNames.BETS} role="button" className="btn btn-primary">
									Голосование
								</Link>
							</p>
						</Col>
				...
				{currentUserBets.map(({ raceId, title, bet }) => {
					return (
